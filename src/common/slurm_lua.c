...
#define LUA_LOG_PREFIX_VAR "lua_script_path"
#define LUA_USER_MSG_VAR "LUA_USER_MSG"

/* Generic stack dump function for debugging purposes */
void slurm_lua_stack_dump (char *header, lua_State *L)
{
#if _DEBUG
	int i;
	int top = lua_gettop(L);

	info("%s: dumping lua stack, %d elements", header, top);
 ...
 *  Optional numeric prefix indicating the
...
static int _log_lua_msg (lua_State *L)
{
	...
	/*
	 * Retrieve the script name from globaldata
	 */
	lua_getglobal(L, LUA_LOG_PREFIX_VAR);
...
{
	lua_setglobal(L, LUA_USER_MSG_VAR);
	return (0);
}
...
/* Returns an allocated string with a message to be returned to the user */
char *slurm_lua_get_user_msg(lua_State *L) {
	char *user_msg = NULL;
	const char *tmp = NULL;

	lua_getglobal(L, LUA_USER_MSG_VAR);
	tmp = lua_tostring(L, -1);
	if (tmp) {
		user_msg = xstrdup(tmp);
		lua_pushnil(L);
		lua_setglobal(L, LUA_USER_MSG_VAR);
	}
	lua_pop(L, 1);
 ...
 */
static int _job_rec_field(lua_State *L, const struct job_record *job_ptr,
                          const char *name)
{
	int i;
...
static int _job_rec_field_index(lua_State *L)
{
	...
	return _job_rec_field(L, job_ptr, name);
}
...
static int _resv_field(lua_State *L, const slurmctld_resv_t *resv_ptr,
                       const char *name)
{
	if (resv_ptr == NULL) {
...
static int _resv_field_index(lua_State *L)
{
	...
	return _resv_field(L, resv_ptr, name);
}
...
static int _job_env_field(lua_State *L, const struct job_descriptor *job_desc,
			  const char *name)
{
	char *name_eq = "";
	int i, name_len;
...
static int _get_job_env_field_name(lua_State *L)
{
	...
	const char *name = luaL_checkstring(L, 2);
	return _job_env_field(L, job_desc, name);
}
...
static void _push_job_env(lua_State *L, struct job_descriptor *job_desc)
{
	lua_newtable(L);
			      ...
			      const char *name)
{
	...
	if (job_desc == NULL) {
		...
		_push_job_env (L, (struct job_descriptor *)job_desc); // No const
...
static int _get_part_rec_field (lua_State *L)
{
	...
	return _part_rec_field(L, part_ptr, name);
}
...
static void _register_lua_slurm_output_functions (lua_State *L)
{
	char *unpack_str;
	char tmp_string[100];
...
static void _register_lua_slurm_struct_functions (lua_State *L)
{
...
/*
 *   Verify all required functions are defined in the lua script
 */
static int _check_lua_script_functions(lua_State *L, const char *lua_script_path, const char *fns[])
{
	int rc = 0;
	int i  = 0;
	do {
		if (_check_lua_script_function(L, fns[i]) < 0) {
			error("lua plugin: %s: "
...
int slurm_lua_load_script(lua_State **LI, const char *lua_script_path, time_t *lua_script_last_loaded, const char *fns[])
{
	...
	if (lua_script_last_loaded != NULL) {
		if (st.st_mtime <= *lua_script_last_loaded) {
			debug2("%s has not updated, not reloading lua state", lua_script_path);
			return SLURM_SUCCESS;
		}
	}
	 ...
	 */
	_register_lua_slurm_output_functions(L);
	_register_lua_slurm_struct_functions(L);
	...
	if (lua_pcall(L, 0, 1, 0) != 0) {
		if (L_orig) {
			(void) error("lua_load_script: %s: %s, "
	...
	if (rc != SLURM_SUCCESS) {
		if (L_orig) {
			(void) error("lua_load_script: %s: returned %d "
		...
		(void) error("lua_load_script: %s: returned %d on load",
		             lua_script_path, rc);
		lua_pop (L, 1);
		return rc;
	}
	 ...
	 */
	rc = _check_lua_script_functions(L, lua_script_path, fns);
	if (rc != SLURM_SUCCESS) {
		if (L_orig) {
			(void) error("lua_load_script: %s: "
	...
	debug("lua plugin loaded %s", lua_script_path);

	/*
	 * Save the script name into a global for logging later
	 */
	lua_pushstring(L, lua_script_path);
	lua_setglobal(L, LUA_LOG_PREFIX_VAR);
